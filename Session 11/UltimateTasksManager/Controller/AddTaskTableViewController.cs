...
namespace UltimateTasksManager.Controller
{
	...
	public partial class AddTaskTableViewController : UITableViewController
	{
		...
		private void ScheduleLocalNotificationForTask (TaskEntity task)
		{
			...
			notification.FireDate = (NSDate) DateTime.Now.AddSeconds (5);
			notification.HasAction = true;
			notification.AlertAction = "Show Detail";
			notification.AlertBody = String.Format("{0}\n{1}", task.Title, task.Description);
			//notification.UserInfo.SetValueForKey (NSObject.FromObject(id), (NSString) "taskId");
		...
		partial void OnSaveButtonTap (UIKit.UIBarButtonItem sender)
		{
			...
			else
			{
				...
				ScheduleLocalNotificationForTask(task);
