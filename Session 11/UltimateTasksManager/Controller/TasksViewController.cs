...
namespace UltimateTasksManager.Controller
{
	public partial class TasksViewController : UIViewController
	{
		...
		private Dictionary<int, int> StateChangeCounters;
		...
		partial void ItemStateChanged (UISwitch sender)
		{
			...
			UIView.AnimateAsync(1.0f, () => {
				//TODO: Set alpha when table is populated
				if(sender.On) {
					parentCell.Alpha = 0.4f;
				}else {
					parentCell.Alpha = 1.0f;
				}
			});
			...
			if(!StateChangeCounters.ContainsKey(entity.Id)) {
				StateChangeCounters.Add(entity.Id, 0);
			}

			StateChangeCounters[entity.Id] = StateChangeCounters[entity.Id] + 1;

			if(StateChangeCounters[entity.Id] > 3) {
				PerformSegue("PongGameViewControllerSegue", this);
			}
		}
		...
		private void SearchHandler(object sender, EventArgs e)
		{
			...
			TasksTableView.ReloadSections(NSIndexSet.FromIndex(0), UITableViewRowAnimation.Fade);
		}
		...
		public override void ViewDidLoad ()
		{
			...
			LoadTasks ();
			StateChangeCounters = new Dictionary<int, int> ();
		}

		public override void ViewWillAppear (bool animated)
		{
			// Reset easter egg dict
			StateChangeCounters.Clear ();
