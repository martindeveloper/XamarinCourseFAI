...
namespace UltimateTasksManager.Controller
{
	...
	partial class TaskDetailViewController : UIViewController
	{
		...
		{
			if (String.IsNullOrWhiteSpace (SelectedTask.Location)) {
				ForecastLabel.Text = "Unknown location";
				return;
			}

			try{
				Weather condition = await WeatherAPI.FetchWeatherForLocationAsync(SelectedTask.Location);
